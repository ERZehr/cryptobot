...
subprocess.run(["bash", "../../bashScripts/functional/getData.sh", ASSET, CURRENCY, str(int(LIMIT) + 13), RANGE])  # Run getData.sh to create a data file

FILENAME = ASSET + "_last_" + str(int(LIMIT) + 13) + "_" + RANGE + "s"  # Create a filename
...
LIMIT = min(LIMIT, total_data_points - 13)  # Adjust LIMIT if it exceeds the available data points

period = 14
highs = [float(row.split(", ")[1]) for row in data]
stochastic_oscillators = []

for i in range(13, 13 + LIMIT):
...
with open(FILENAME2, 'w') as file:
    for i, oscillator in enumerate(stochastic_oscillators, start=(period-1)):
        file.write(f"{data[i].split(', ')[0]}, {oscillator}\n")
