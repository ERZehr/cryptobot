...
for item in DATA:
    if 'time' in item and 'close' in item:
        dt = datetime.datetime.fromtimestamp(item['time'])  # Convert timestamp to a datetime object
        list_of_lists.append([str(dt), str(item['close'])])  # Convert the 'close' value to a string

filename = ASSET + "_" + "last_" + LIMIT + "_" + RANGE + "s"  # Generate the filename
